...
const DATA_FOLDER: &str = "dbs";
const FAILURE_TO_OPEN: &str = "Could Not Open Database";
const FAILURE_TO_CLOSE: &str = "Could Not Close The Database";
const FAILURE_TO_INSERT: &str = "Could Not Insert Data";
...
/// There is a new doctrine of this database module
/// That is that it will automatically drop any database
/// connections to save it into the file.
/// It makes things simpler and also easier when testing.
impl DatabaseHandler {
    pub fn initialize_database(database_name: &String, mut tables: Vec<Table>) {
        ...
        let pre_tables: Vec<String> = if exists {
            DatabaseHandler::get_tables(database_name)
        } else {
            vec![]
        };
        ...
        for table in &tables {
            DatabaseHandler::create_table(database_name, table.name.clone(), table.data_type.clone());
        }
