...
pub fn initialize_database(database_name: &String, mut tables: Vec<Table>) {
    ...
    for table in &tables {
        create_sensor_table(database_name, table.name.clone(), table.data_type.clone());
    }
...
pub fn create_sensor_table(database_name: &String, table_name: String, value_type: String) {
